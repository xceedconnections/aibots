...
def vicidial_ip_peer_snippet(aibots_public_ip: str, peer_name: str = "aibots") -> str:
    ...
    return (
        ...
        f"\n"
        f"; Add to extensions.conf (Vicibox):\n"
        f"; [from-aibots]\n"
        f"; ; Virtual DIDs from AIBOTS transfer (example)\n"
        f"; exten => _1060XXXXXX,1,Goto(default,${{EXTEN}},1)\n"
        f"; exten => _X.,1,Goto(default,${{EXTEN}},1)\n"
        f"; exten => _.,1,NoOp(Ignore AIBOTS signalling ${{EXTEN}})\n"
        f";  same => n,Hangup()\n"
    )
