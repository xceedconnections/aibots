...
def vicidial_ip_peer_snippet(aibots_public_ip: str, peer_name: str = "aibots") -> str:
    ...
    return (
        ...
        f"; CRITICAL: do NOT set username= or secret=\n"
        f"; context=from-aibots must accept transfer DIDs (and ignore junk like IP URIs)\n"
        f"[{peer_name}]\n"
        f"host={aibots_public_ip}\n"
        f"type=peer\n"
        f"context=from-aibots\n"
        f"disallow=all\n"
        f"allow=ulaw\n"
        ...
        f"\n"
        f"; Vicidial Admin → System Settings → Custom Dialplan Entry:\n"
        f"; [from-aibots]\n"
        f"; ; ONLY transfer DIDs — do NOT add exten => _X. Goto(default) (causes ss-noservice)\n"
        f"; exten => _1060XXXXXX,1,Goto(default,${{EXTEN}},1)\n"
        f"; exten => _.,1,NoOp(Ignore AIBOTS ${{EXTEN}})\n"
        f";  same => n,Hangup()\n"
    )
