...
async def seed_sample_bot(db: AsyncSession) -> Bot:
    """Create ACA Qualifier bot if missing. Returns the bot (existing or new)."""
    existing = await db.execute(select(Bot).where(Bot.name == "ACA Qualifier"))
